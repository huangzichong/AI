...
def get_batch(class_num,size_each_class,image_size=(240,240)):
    ...
    result=[0.00 for i in range(0,class_num)]
    g=[[0.00 for j in range(0,class_num)]for i in range(0,class_num)]
    for i in range(0,class_num):
        y[i]=str(i+1)+'_'+str(random.randint(1,size_each_class))
        image_file = os.path.join('imageForTest',y[i]+'.bmp')   #images/car2.jpg
        image2 = cv2.imread(image_file)  #read the image, images/car2.jpg
        result[i]=preprocess_image(image2,image_size)
        g[i][int(class_num-i-1)]=1.0
...
for i in range(1,4):
    for j in range(1,58):
        image_file = os.path.join('imageForTest',str(i)+'_'+str(j)+'.bmp')   #images/car2.jpg
        image2 = cv2.imread(image_file)  #read the image, images/car2.jpg
        x=preprocess_image(image2,image_size=(240,240))
        x=np.expand_dims(x,axis=0)
        x=np.expand_dims(x,axis=3)

        a=conv(x,w)  #234x234
        a_n=batch_normalization(a,norma,mode='test')
        a_=relu(a_n)
        a__,a_pool_der=maxpool(a_,step=2) #117x117

        b=conv(a__,w2,padding=0) #114x114
        b_n=batch_normalization(b,norma2,mode='test')
        b_=relu(b_n)

        b__,b_pool_der=maxpool(b_,step=2) #57x57

        c=conv(b__,w3) #54x54
        c_n=batch_normalization(c,norma3,mode='test')
        c_=relu(c_n)
        c__,c_pool_der=maxpool(c_,step=2) #27x27

        d=conv(c__,w4) #20x20
        d_n=batch_normalization(d,norma4,mode='test')
        d_=relu(d_n)
