...
w5=np.random.normal(0,1,(10,1,1,10))*np.sqrt(2/(10+1)) #10x10

adam_w=adamOp(w,0.01)
adam_w2=adamOp(w2,0.007)
adam_w3=adamOp(w3,0.006)
adam_w4=adamOp(w4,0.002)
adam_w5=adamOp(w5,0.002)
...
for i in range(0,1000):
    ...
    '''
    x,y=get_batch(3,57)
    y=np.zeros([15,10,10,2,5])
    y[:,:,:,:,0:2]=1.0
    a=conv(x,w)  #234x234
    a_n=batch_normalization(a,norma2,mode='train')
    a_=relu(a_n)
    a__,a_pool_der=maxpool(a_,step=2) #117x117
    
    b=conv(a__,w2,padding=0) #114x114
    b_n=batch_normalization(b,norma3,mode='train')
    b_=relu(b_n)
    b__,b_pool_der=maxpool(b_,step=2) #57x57
    
    c=conv(b__,w3) #54x54
    c_n=batch_normalization(c,norma4,mode='train')
    c_=relu(c_n)
    c__,c_pool_der=maxpool(c_,step=2) #27x27

    d=conv(c__,w4) #20x20
    d_n=batch_normalization(d,norma5,mode='train')
    d_=relu(d_n)
    d__,d_pool_der=maxpool(d_,step=2) #10x10

    dnn=conv(d__,w5)#10x10
    dnn_= dnn.reshape([dnn.shape[0],dnn.shape[1],dnn.shape[2],2,5])
    dnn__=sigmoid(dnn_)
    print(dnn__[:,:,:,:,0:5])
    cost=cost_derivative(dnn__,y)
    cita1=sigmoid_derivative(dnn__)*cost
    cita1=cita1.reshape([dnn.shape[0],dnn.shape[1],dnn.shape[2],dnn.shape[3]])
    
    deta=conv_back(d__,cita1)
    cita2=conv_cita(cita1,w5)
    cita2=relu_derivative(d_n,d_)*np.repeat(np.repeat(cita2,2,1),2,2)*d_pool_der
    cita2=batch_normalization_back(norma5,cita2)
    w5=adam(adam_w5,w5,deta)
    ...
    cita3=batch_normalization_back(norma4,cita3)
    ...
    cita4=batch_normalization_back(norma3,cita4)
    ...
    cita5=batch_normalization_back(norma2,cita5)
    ...
    if i%100==0:
       print("save")
       np.savez("fastercnn_weight.npz", w=w, w2=w2, w3=w3, w4=w4,w5=w5)
       np.savez("fastercnn_normaOp.npz",norma2=norma2, norma3=norma3, norma4=norma4, norma5=norma5)
