...
def compute_loss(predictions,targets,anchors,scales,num_classes=20,output_size=(13,13)):  #损失函数
    ...
    #predictions = tf.reshape(predictions,[-1,H*W,B,5+C]) #测试用
    ...
    # 置信度矩阵取反
    confs_wTemp = 0.0 -_confs+1.0
    confs_wTemp = np.reshape(confs_wTemp,[-1,H*W,B,1])
    confs_wTemp = confs_wTemp+ mask
    confs_loss = preds[0,99,0]
    confs_w = confs_wTemp#snoob * (1 - mask) + sconf * mask   #分别计算出没有目标的置信度和有目标的置信度， [-1,H*W,B,1]
    ...
    #confs_loss = tf.pow(preds[:,99,4] - truths[:,99,4], 2)
    loss = tf.pow(preds-truths, 2) * weights
     
    #loss = tf.pow(preds-truths, 2)
    #loss = tf.reduce_sum(loss, axis=[1, 2, 3]) #测试时暂时屏蔽掉
    ...
    return train_op,loss,predictionsTemp,confs_loss
