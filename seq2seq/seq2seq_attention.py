...
def decoding_layer(target_letter_to_int, decoding_embedding_size, num_layers, rnn_size,
    ...
    def get_decoder_cell(rnn_size):
        ...
        decoder_cell = tf.contrib.rnn.DropoutWrapper(decoder_cell,input_keep_prob=1.0,output_keep_prob=0.8)
        return decoder_cell
    
    cell = tf.contrib.rnn.MultiRNNCell([get_decoder_cell(rnn_size) for _ in range(num_layers)])
    #注意力机制
   
    attension_menchian = tf.contrib.seq2seq.LuongAttention(rnn_size, encoder_output)
...
with train_graph.as_default():
    ...
    cost = tf.contrib.seq2seq.sequence_loss(
        ...
        masks,
        name='cost')
    costTemp = tf.identity(cost, 'costTemp')
    # Optimizer
    ...
    train_op = optimizer.apply_gradients(capped_gradients,name='train_op')    #启用梯度剪裁
