...
with open('data/letters_source_real.txt', 'r', encoding='utf-8') as f:
    source_data = f.read()

with open('data/letters_target_real.txt', 'r', encoding='utf-8') as f:
    target_data = f.read()
...
def extract_character_vocab(data):
    ...
    set_words = list(set([character for line in data.split('\n') for character in " ".join(jieba.cut(line)).split(' ')]))
    #这里要把四个特殊字符添加进词典
    int_to_vocab = {idx+1: word for idx, word in enumerate(special_words + set_words)}
    vocab_to_int = {word: idx for idx, word in int_to_vocab.items()}
    return int_to_vocab, vocab_to_int
    '''
...
source_int = [[source_letter_to_int.get(letter, source_letter_to_int['<UNK>']) 
               for letter in " ".join(jieba.cut(line)).split(' ')] for line in source_data.split('\n')]
target_int = [[target_letter_to_int.get(letter, target_letter_to_int['<UNK>']) 
               for letter in " ".join(jieba.cut(line)).split(' ')] + [target_letter_to_int['<EOS>']] for line in target_data.split('\n')]
...
def get_encoder_layer(input_data, rnn_size, num_layers,
    ...
    cells = tf.contrib.rnn.MultiRNNCell([get_lstm_cell(rnn_size) for _ in range(num_layers)])
    
    encoder_output, encoder_state_temp = tf.nn.bidirectional_dynamic_rnn(cell,cells, encoder_embed_input, 
                                                      sequence_length=source_sequence_length, dtype=tf.float32)
    encoder_state = encoder_state_temp[0]
...
with train_graph.as_default():
    ...
    print(len(target_letter_to_int))
    ...
    with tf.name_scope("optimization"):
        ...
        capped_gradients = [(tf.clip_by_value(grad, -0.6, 0.6), var) for grad, var in gradients if grad is not None]  #梯度剪裁
        train_op = optimizer.apply_gradients(capped_gradients)    #启用梯度剪裁
...
train_source = source_int[1:]
train_target = target_int[1:]

# 留出一个batch进行验证
valid_source = source_int[1:]
valid_target = target_int[1:]
