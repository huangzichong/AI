...
def get_image(index):
    ...
    if os.path.isfile(path):
       I = Image.open(path)
       tmp = list[index].split('.')
       name=tmp[0]
       image=I
    return image,name,'333'
...
def get_next_batch(batch_size=BATCH_SIZE):
    ...
    for i in range(batch_size):
        ...
        image, text, vec = get_image(i)
        #np.transpose 矩阵转置 (32*256,) => (32,256) => (256,32)
        inputs[i,:] = normalize(np.transpose(np.array(image)), axis=1, norm='max')
...
def train():
    ...
    optimizer = tf.train.AdamOptimizer(learning_rate=learning_rate).minimize(loss,global_step=global_step,name='Adam')
    decoded, log_prob = tf.nn.ctc_beam_search_decoder(logits, seq_len, merge_repeated=False)
    
    acc = tf.reduce_mean(tf.edit_distance(tf.cast(decoded[0], tf.int32), targets),name='acc')
    ...
    checkpoint = "./trained_modelTestCtc.ckpt" 
    with tf.Session() as session:
        ...
        for curr_epoch in range(num_epochs):
            ...
            decodeds,val_cost, val_ler,steps = session.run([decoded,cost, acc, global_step], feed_dict=val_feed)
            print(decodeds)
            log = "Epoch {}/{}, steps = {}, train_cost = {:.3f}, train_ler = {:.3f}, val_cost = {:.3f}, val_ler = {:.3f}, time = {:.3f}s, learning_rate = {}"
            print(log.format(curr_epoch + 1, num_epochs, steps, train_cost, train_ler, val_cost, val_ler, time.time() - start, learning_rate))
        # 保存模型
